...
async def get_year_contents(
    year: int, depends: DependsData = Depends(get_content_table_and_user_id)
) -> List[dict]:
    try:
        content_items = [
            ContentData(**item)
            for item in get_year_contents_service(
                depends.user_id, depends.table, year
            )
        ]
