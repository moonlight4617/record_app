...
def logout(response: Response):
    try:
        if ENVIRONMENT == "production":
            ...
            response.delete_cookie(key="id_token", path="/", httponly=True, secure=True)
            response.delete_cookie(key="refresh_token", path="/", httponly=True, secure=True)
            response.delete_cookie(key="user_id", path="/", httponly=True, secure=True)
        else:
            response.delete_cookie(key="access_token", httponly=True, samesite='Lax', secure=False)
            response.delete_cookie(key="id_token", httponly=True, samesite='Lax', secure=False)
            response.delete_cookie(key="refresh_token", httponly=True, samesite='Lax', secure=False)
            response.delete_cookie(key="user_id", httponly=True, samesite='Lax', secure=False)
