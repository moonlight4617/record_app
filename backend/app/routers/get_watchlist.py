...
async def get_watchlist(depends: DependsData = Depends(get_content_table_and_user_id)) -> List[dict]:
    try:
        content_items = [watchlistData(**item) for item in get_watchlist_service(depends.user_id, depends.table)]
