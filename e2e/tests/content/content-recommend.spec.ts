...
test.describe("レコメンド機能フロー", () => {
  test("許可されたユーザーのみがレコメンド機能を使用できる", async ({ page }) => {
    ...
    try {
      ...
      const recommendPage = page.locator("a[href*='recommend'], button[data-testid='recommend']").first();
      ...
      if (recommendElementFound) {
        ...
        try {
          ...
          const resultsVisible = await recommendResults.isVisible({ timeout: 3000 });
          const itemsCount = await recommendItems.count();
          
          if (resultsVisible || itemsCount > 0) {
            console.log(`Success: レコメンド機能が動作し、${itemsCount}件の結果が表示されました`);
            ...
            console.log("レコメンド結果は表示されませんでしたが、機能は利用可能です");
            await page.screenshot({ path: "test-results/recommend-no-results.png" });
          }
          
        } catch (error) {
          console.log("レコメンド結果の確認中にエラーが発生:", error);
          await page.screenshot({ path: "test-results/recommend-results-error.png" });
        }
        ...
        try {
          ...
          page.on('response', (response) => {
            ...
            if (url.includes('recommend') || url.includes('suggestion') || url.includes('api')) {
          ...
          if (apiCalls.length > 0) {
            console.log("レコメンド関連のAPI呼び出しを確認:", apiCalls.join(", "));
          } else {
        ...
        await page.screenshot({ path: "test-results/recommend-feature-not-found.png" });
        
        // 現在のページの構造を確認
        const pageText = await page.textContent("body");
        console.log("現在のページに含まれるテキストの一部:", pageText?.substring(0, 200) || "取得できませんでした");
      }
    ...
    try {
      ...
      if (currentUrl.includes("/login") || currentUrl.includes("/auth")) {
        console.log("Success: 未認証ユーザーはログインページにリダイレクトされました");
      } else if (pageText?.includes("ログイン") || pageText?.includes("認証")) {
        console.log("Success: 未認証ユーザーには認証を求めるメッセージが表示されました");
      } else {
        console.log("Warning: 未認証ユーザーでもレコメンド機能にアクセスできるようです");
        await page.screenshot({ path: "test-results/recommend-unauthorized-access.png" });
      }
      
    } catch (error) {
      console.log("権限テスト中にエラーが発生:", error);
      await page.screenshot({ path: "test-results/recommend-permission-test-error.png" });
    }
